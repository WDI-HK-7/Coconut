...
# before_action :authenticate_user!
  ...
  def live_feed(postId)
    ...
    @posts = Post.order(taken_at: :desc).where("latitude > ?" , post.latitude - 0.001).where("latitude < ?" , post.latitude + 0.001).where("longitude > ?" , post.longitude - 0.001).where("longitude < ?" , post.longitude + 0.001).where("taken_at > ?", post.taken_at - 1.hour)
  end
